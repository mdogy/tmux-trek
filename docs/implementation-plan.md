...
- Current verified local baseline (July 17, 2026, PR #22 branch): `npm run lint` ✓, `npm run test` (113 unit tests) ✓, `npm run bdd` (2 scenarios / 17 steps) ✓, `npm run build` ✓, `npm run test:e2e:desktop` (11 Playwright tests incl. the full vertical slice) ✓, `npm run test:e2e:mobile` (63 tests: 21 specs × Pixel 7 / iPhone 14 / iPad gen 7 device profiles) ✓ twice consecutively. Earlier coverage and stress checks remain documented in [`session-handoff.md`](session-handoff.md).
...
to face the viewscreen at half scale, the captain starts directly in front of it facing up,
Comms is aligned back-left to its console, the First Officer stands at the forward console area,
the bridge title sits above the viewscreen, and the Rift terminal now lives on the free
starboard-center bridge console rather than the far-right wall. Full critique, per-zone
...
- The bridge layout now matches the corrected artwork: half-scale chair/captain orientation,
  crew station placement, viewscreen title placement, and central Rift terminal interaction are
  covered by `ZoneLoader` and Playwright bridge-art tests.
...
**Current reality check (July 17, 2026):** the shell and overlays collapse to one column under `980px`, the Phaser canvas uses `Scale.FIT` and now re-fits on rotation, font-load reflow, and layout shifts (the scale manager refreshes on `document.fonts.ready`, a body `ResizeObserver`, and window resize — this also keeps Phaser's tap-coordinate translation correct). The boot loading overlay no longer swallows the first tap, and review/dialogue buttons honor the 44px touch minimum at every viewport. All of this is guarded by the **mobile usability suite** (`tests/e2e/mobile/`, see [`mobile-testing.md`](../mobile-testing.md)): 21 specs against Pixel 7 (Chromium), iPhone 14 (WebKit), and iPad gen 7 (WebKit) profiles with real touch events, covering title/save/review touch flows, rotation, layout fit, terminal focus, and persistence. Known tracked gap: title-menu hit zones are ~20px on phones (expected-failure test). Capability-based routing into Review Mode (Tier 2, Checkpoint 3) is still not implemented. Today the product is mobile-viable only with a real keyboard; touch-only support remains an honest review-first product decision, not a completed feature.
